...
def get_data(dataset_size, *, key, func=None, t_end=1, n_points=100):
    ...
    bounds = [(1.0, 2.5), (1.0, 2.5), (0.5, 0.8), (0.5, 0.8),]
    bounds = jnp.repeat(jnp.array(bounds)[None, :], dataset_size, axis=0)   
    ys, params = jax.vmap(solveLVE)(ts, y0, bounds, key_dataset)
...
def main(
    ...
    LVE_args = (1.6, 1.6, 0.9, 0.9)  # a=prey-growth, b, c, d
    ...
    for step, (ts_i, ys_i) in zip(
        ...
        if (step % save_every) == 0 or step == steps - 1:
            fn = SAVE_DIR + "/" + save_name + "seq1"  + save_suffix + "_step_" + str(step) + ".eqx"
    ...
    # get the data
    ts, ys = get_data(
        dataset_size, key=data_key, func=func, t_end=35, n_points=n_points
    )
...
main(
    ...
    t_final=20,  # final time of the ODE (note this is randomised between t_final and 2*t_final)
